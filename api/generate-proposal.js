...
let intakeConversation = null;    // Stored in memory for the Supabase transcript column
...
async function renderProposalPdf({ company_name, contact_name, sections }) {
  ...
  // CfCE brand palette: #1E2D40 (primary) · #1D9E75 (accent) · #FAC775 (pop)
  const brandPrimary = rgb(0.118, 0.176, 0.251); // #1E2D40 steel blue-gray
  const brandAccent  = rgb(0.114, 0.620, 0.459); // #1D9E75 teal
  ...
  cover.drawText('Coding for Civil Engineers', {
    x: 50, y: 732, size: 22, font: fontBold, color: rgb(1, 1, 1),
  });
  cover.drawText('Automate STAAD. Reclaim your time.', {
  ...
  lastPage.drawText('jparishith@gmail.com  |  codingforcivilengineers.com', {
...
async function storeLead(leadData) {
  ...
  const row = {
    ...
    conversation_transcript: intakeConversation,
  };
...
const AGENT_SYSTEM_PROMPT = `You are an AI agent acting on behalf of Parishith Jayan, founder of Coding for Civil Engineers (CfCE) at codingforcivilengineers.com.

You have received intake data from a website visitor who requested a proposal. Your job:
1. Write a personalized proposal in Parishith's voice
2. Score the lead using the triage rules below
3. Use your tools to: render the proposal as a PDF, email it to the visitor, store the lead (if store_lead tool is available), and alert Parishith on Telegram
...
- Short punchy sentence after a longer one. Use it to land the key point.
- Use naturally: "Is not it?" as a question tag. "trust me" as a confidence marker. "Being said that" as a transition. "Let us" to pull the reader in.
- Practitioner authority, not academic distance. Every claim grounded in real project experience. Warm and credible, never stiff.
- Never use: "Best regards", "Leverage", "Synergy", hedging phrases, or passive voice.

## LEAD SCORING RULES

Score HIGH if the lead shows any of the following:

Corporate signals (automatic HIGH):
- Mentions a firm name, team size, or multiple engineers
- References standardising workflows, training their team, or rolling out automation across projects
- Title is Engineering Manager, Head of Engineering, Principal Engineer, or similar

Individual signals (strong buyer intent):
- Practising structural engineer at a firm using STAAD on live projects
- Describes a specific automation pain: load combination generation, member force extraction, result export, report generation, model creation
- Mentions they have tried workarounds and are frustrated — Excel, manual copy-paste, repetitive tasks
- Already familiar with Parishith's content (LinkedIn, YouTube, previous cohort) — warm, trust already built
- Ready language: "want to join", "how do I enrol", "next batch", "is it open"

Score MEDIUM if:
- Engineer interested in automation but challenge is vague — "want to learn coding" without a specific pain
- No firm mentioned but clearly working (not a student) — motivation real but context thin
- Junior engineer — intent genuine but may need approval or is budget-sensitive at Rs. 15,000
- Asking exploratory questions about curriculum, time commitment, prerequisites — evaluating, not decided
- Budget uncertain ("depends", "let me check")
...
Tiebreaker: does this person lose real time or money today because of the problem they described? If yes, HIGH.
...
## INSTRUCTIONS
- Write the proposal in Parishith's voice — direct, personal, specific to their situation
- Score the lead (HIGH/MEDIUM/LOW) using the triage rules
- Call render_proposal_pdf with company_name, contact_name (use email if no name), and sections
- Call send_email: warm 2-3 sentence intro email, PDF attached, from Parishith
- If store_lead tool is available, call it with all lead data and score
- Call alert_owner with: company, contact email, challenge summary, score, and one line on why you scored it that way
- You decide the order. render_proposal_pdf must complete before send_email (PDF attachment needed). Other tools can run in parallel.`;
...
module.exports = async function handler(req, res) {
  ...
  intakeConversation = Array.isArray(conversation)
